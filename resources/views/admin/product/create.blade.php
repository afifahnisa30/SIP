    ...
    <div class="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
        ...
        <form action="{{ route('product.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="ukuran_standar" value="{{ old('ukuran_standar') }}" placeholder="Contoh jika banyak: 2,3,4 (pisahkan dengan koma)"
                    class="w-full py-3 px-5 text-gray-700 bg-slate-50 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition text-sm">
                <span class="text-xs text-gray-400 mt-1 block">*Khusus produk Spanduk. Kosongkan jika bukan spanduk.</span>
            </div>

            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">Deskripsi / Keterangan Produk</label>
                <textarea name="deskripsi" rows="4" placeholder="Tulis informasi detail mengenai bahan atau minimal order..."
                    class="w-full py-3 px-5 text-gray-700 bg-slate-50 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition text-sm">{{ old('deskripsi') }}</textarea>
            </div>
