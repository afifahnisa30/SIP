...
<div class="flex items-center justify-between mb-6">
    <div>
        ...
        <p class="text-xs text-gray-400 mt-1">Rekap transaksi dan pengeluaran harian</p>
    </div>
</div>

{{-- PILIH TANGGAL --}}
<form method="GET" action="{{ route('laporan.harian') }}" class="flex gap-3 mb-6">
...
@if($tanggal)
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-slate-50 transition">
                            ...
                            <td class="px-5 py-3">
                                <span class="px-2 py-1 rounded-full text-xs font-medium {{ $order->metode_bayar == 'Tunai' ? 'bg-amber-0 text-amber-600' : 'bg-purple-50 text-purple-600' }}">
                                    {{ $order->metode_bayar }}
                                </span>
                            </td>
                        ...
                        @empty
                        <tr><td colspan="6" class="px-5 py-10 text-center text-gray-400 text-xs"><i class="fas fa-inbox text-2xl mb-2 block"></i>Tidak ada transaksi</td></tr>
                        @endforelse
                    </tbody>
                    @if($orders->count() > 0)
                    <tfoot>
                        <tr class="bg-emerald-50">
                            <td colspan="5" class="px-5 py-3 text-sm font-bold text-gray-700 text-right">Total Pendapatan</td>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        @empty
                        <tr><td colspan="5" class="px-5 py-10 text-center text-gray-400 text-xs"><i class="fas fa-receipt text-2xl mb-2 block"></i>Tidak ada pengeluaran</td></tr>
                        @endforelse
                    </tbody>
                    @if($pengeluaran->count() > 0)
                    <tfoot>
                        <tr class="bg-red-50">
                            <td colspan="4" class="px-5 py-3 text-sm font-bold text-gray-700 text-right">Total Pengeluaran</td>
    ...
    {{-- ===== KONTEN PRINT (tersembunyi di layar) ===== --}}
    <div class="hidden print:block">
        <div class="print-header">
            ...
            <table style="font-size:10px; text-align:left; border:none;">
                <tr>
                    <td style="padding:1px 4px;">Laporan tanggal</td>
                    <td style="padding:1px 4px;">:</td>
                    <td style="padding:1px 4px;"><strong>{{ \Carbon\Carbon::parse($tanggal)->translatedFormat('d F Y') }}</strong></td>
                </tr>
                <tr>
        ...
        <table>
            <thead>
                <tr>
                    <th>No</th><th>Pelanggan</th><th>Produk</th><th>Detail</th>
        ...
        <table>
            <thead>
                <tr>
                    <th>No</th><th>Kategori</th><th>Keterangan</th>
                    <th style="text-align:right">Total</th><th>Pembayaran</th>
                </tr>
            </thead>
            <tbody>
                ...
                @empty
                <tr><td colspan="5" style="text-align:center">Tidak ada pengeluaran</td></tr>
                @endforelse
            </tbody>
            <tfoot>
                <tr>
...
@else
    <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-16 text-center text-gray-400">
        ...
        <p class="font-semibold text-gray-500">Pilih tanggal dan tekan Tampilkan</p>
